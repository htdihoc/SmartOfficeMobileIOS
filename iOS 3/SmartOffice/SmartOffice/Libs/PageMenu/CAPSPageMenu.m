...
@interface CAPSPageMenu () <SO_TitleSegmentDelegate>
{
    SO_TitleSegment *_segmentTitleView;
}
@property (nonatomic) CGFloat widthOfCustomizeTitleLabel;
...
NSString * const CAPSWidthOfCustomizeTitleLabel                       = @"widthOfCustomizeTitleLabel";
- (instancetype)initWithViewControllers:(NSArray *)viewControllers frame:(CGRect)frame options:(NSDictionary *)options
{
    ...
    if (options) {
        for (NSString *key in options) {
            ...
            else if ([key isEqualToString:CAPSWidthOfCustomizeTitleLabel])
            {
                _widthOfCustomizeTitleLabel = [options[key] floatValue];
            }
        }
...
- (void)addSegmentTitle:(NSString *)firstTitle second:(NSString *)secondTitle third:(NSString *)thirdTitle
{
    CGFloat defaultSegmentHeight = _menuHeight;
    _segmentTitleView = [[SO_TitleSegment alloc] initWithFrame:self.view.frame];
        [_segmentTitleView setTitleForButtons:firstTitle second:secondTitle third:thirdTitle];
    [_segmentTitleView setFontForButtons:_menuItemFont];
    _segmentTitleView.backgroundColor = AppColor_MainAppTintColor;
    [self.view addSubview:_segmentTitleView];
    _segmentTitleView.translatesAutoresizingMaskIntoConstraints = NO;
    _segmentTitleView.delegate = self;
    NSLayoutConstraint *topSegment = [NSLayoutConstraint
                                      constraintWithItem:_segmentTitleView
                                      ...
                                      multiplier:1.0
                                      constant:0];
    NSLayoutConstraint *heightSegment = [NSLayoutConstraint
                                         constraintWithItem:_segmentTitleView
                                         attribute:NSLayoutAttributeHeight
                                         relatedBy:NSLayoutRelationEqual toItem:nil
                                         attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0f
                                         constant:defaultSegmentHeight - _selectionIndicatorHeight];
    NSLayoutConstraint *leftSegment = [NSLayoutConstraint
                                       constraintWithItem:_segmentTitleView
                                       ...
                                       constant:0];
    NSLayoutConstraint *rightSegment = [NSLayoutConstraint
                                        constraintWithItem:_segmentTitleView
    ...
    [self.view addConstraints:@[topSegment, leftSegment, heightSegment, rightSegment]];
    
}
#pragma mark SO_TitleSegmentDelegate
...
- (void)initValues
{
    ...
    _widthOfCustomizeTitleLabel       = 0.0;
...
- (void)configureUserInterface
{
    ...
    for (UIViewController *controller in _controllerArray) {
        ...
        if (_useMenuLikeSegmentedControl) {
            CGFloat widthView = _widthOfCustomizeTitleLabel != 0 ? _widthOfCustomizeTitleLabel : self.view.frame.size.width;
            menuItemFrame = CGRectMake(widthView / (CGFloat)_controllerArray.count * (CGFloat)index, 0.0, (CGFloat)widthView / (CGFloat)_controllerArray.count, _menuHeight);
    ...
    if (_controllerArray.count == 3) {
        [self addSegmentTitle:((UIViewController *)_controllerArray[0]).title second:((UIViewController *)_controllerArray[1]).title third:((UIViewController *)_controllerArray[2]).title];
    }
    ...
    if (!_didLayoutSubviewsAfterRotation) {
        if ([scrollView isEqual:_controllerScrollView]) {
            if (scrollView.contentOffset.x >= 0.0 && scrollView.contentOffset.x <= ((CGFloat)(_controllerArray.count - 1) * self.view.frame.size.width)) {
                ...
                if (_controllerScrollView.contentOffset.x < -5) {
                    if ([_delegate respondsToSelector:@selector(popBack)]) {
                        [_delegate popBack];
                    }
                }
            }
    ...
    if (pageIndex >= 0 && pageIndex < _controllerArray.count) {
        [UIView animateWithDuration:0.15 animations:^{
            ...
            if (self.useMenuLikeSegmentedControl) {
                selectionIndicatorX = (CGFloat)pageIndex * (_widthOfCustomizeTitleLabel != 0 ? _widthOfCustomizeTitleLabel : (self.view.frame.size.width / (CGFloat)_controllerArray.count));
                selectionIndicatorWidth = _widthOfCustomizeTitleLabel != 0 ? _widthOfCustomizeTitleLabel : self.view.frame.size.width / (CGFloat)_controllerArray.count;
...
- (void)handleMenuItemTap:(UITapGestureRecognizer *)gestureRecognizer
{
    ...
    if (tappedPoint.y < self.menuScrollView.frame.size.height) {
        ...
        if (_useMenuLikeSegmentedControl) {
            itemIndex = (NSInteger) (tappedPoint.x / (_widthOfCustomizeTitleLabel != 0 ? _widthOfCustomizeTitleLabel : (self.view.frame.size.width / (CGFloat)_controllerArray.count)));
        } else if (_menuItemWidthBasedOnTitleTextWidth) {
...
- (void)addPageAtIndex:(NSInteger)index
{
    ...
    if ([_delegate respondsToSelector:@selector(willMoveToPage:index:)]) {
        [_segmentTitleView touchAt:index];
...
- (void)viewDidLayoutSubviews
{
    ...
    if ((oldCurrentOrientationIsPortrait && UIInterfaceOrientationIsLandscape(orientation)) || (!oldCurrentOrientationIsPortrait && UIInterfaceOrientationIsPortrait(orientation))){
        ...
        if (_useMenuLikeSegmentedControl) {
            ...
            CGFloat selectionIndicatorX = (CGFloat)_currentPageIndex * (_widthOfCustomizeTitleLabel != 0 ? _widthOfCustomizeTitleLabel : (self.view.frame.size.width / (CGFloat)_controllerArray.count));
            CGFloat selectionIndicatorWidth = _widthOfCustomizeTitleLabel != 0 ? _widthOfCustomizeTitleLabel : self.view.frame.size.width / (CGFloat)_controllerArray.count;
            ...
            for (MenuItemView *item in _mutableMenuItems) {
                ...
                if (item.titleLabel) {
                    CGFloat xPosition = 0;
                    if(_widthOfCustomizeTitleLabel != 0)
                    {
                        xPosition = index == 0 ? 0 : _widthOfCustomizeTitleLabel - self.view.frame.size.width / (CGFloat)_controllerArray.count;
                    }
                    item.titleLabel.frame = CGRectMake(xPosition, 0.0, _widthOfCustomizeTitleLabel != 0? _widthOfCustomizeTitleLabel:self.view.frame.size.width / (CGFloat)_controllerArray.count, _menuHeight);
                }
