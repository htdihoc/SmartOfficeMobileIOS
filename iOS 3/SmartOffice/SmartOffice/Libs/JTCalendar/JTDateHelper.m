...
typedef enum WeekDay : NSUInteger {
    SUN = 1,
    MON,
    TUE,
    WED,
...
- (BOOL)isAnotherMonth:(NSDate *)date increaseMonth:(NSInteger)increaseMonth
{
    NSDateComponents *currentComponents = [[NSCalendar currentCalendar] components: NSCalendarUnitMonth fromDate:[NSDate date]];
    [currentComponents setMonth:[currentComponents month]+increaseMonth];
    NSDateComponents *checkComponents = [[NSCalendar currentCalendar] components: NSCalendarUnitMonth fromDate:date];
    if([currentComponents month] == [checkComponents month])
    {
        return false;
    }
    return true;
...
{
    NSInteger currentWeekDay = [self getCurrentWeekDay: date];
