...
- (BOOL)dateEnabled:(NSDate *)date {
    ...
    if ([self.delegate calendarViewShouldSelectEndDate]) {
        return date.timeIntervalSince1970 >= _startDate.timeIntervalSince1970;
    }
    else
    {
        NSDate *currentDate = [[NSDate date] mn_beginningOfDay:self.calendar];
        BOOL check = date.timeIntervalSince1970 >= currentDate.timeIntervalSince1970;
        return check;
    }
...
- (BOOL)canSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    BOOL enabled = [self dateEnabled:[self getDateAt:indexPath]];
    return enabled;
}
- (NSDate *)getDateAt:(NSIndexPath *)indexPath
{
    ...
    components.day += day;
    return [[self.calendar dateFromComponents:components] mn_beginningOfDay:self.calendar];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
    ...
    NSDate *monthDate = self.monthDates[indexPath.section];
    NSDate *date = [self getDateAt:indexPath];
    [cell setDate:date
            month:monthDate
         calendar:self.calendar];
    
    if (cell.enabled && _endDate != nil) {
        if ([self getDateAt:indexPath] == _endDate) {
            cell.titleLabel.textColor = [UIColor whiteColor];
            [cell setBackgroundColor:AppColor_MainAppTintColor];
        }
        else if ([self getDateAt:indexPath] < _endDate && [self getDateAt:indexPath] >= _startDate) {
            cell.titleLabel.textColor = [UIColor whiteColor];
            [cell setBackgroundColor:CommonColor_Blue];
        }
    }
    
    if (self.selectedDate && cell.enabled) {
        if (_endDate == nil && [date isEqualToDate:self.selectedDate]) {
            _startDate = [self getDateAt:indexPath];
        }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if (self.delegate && [self.delegate respondsToSelector:@selector(calendarViewShouldSelectEndDate)]){
        if ([self.delegate calendarViewShouldSelectEndDate]) {
            _endDate = [self getDateAt:indexPath];
        }
        else
        {
            _endDate = nil;
