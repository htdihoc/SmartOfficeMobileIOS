...
- (id)init
{
    ...
    if (self) {
        ...
        self.titleAlight = NSTextAlignmentCenter;
        ...
        _customImageScale = 1;
        _titleFont = [UIFont systemFontOfSize:16.0f weight:UIFontWeightMedium];
        _subtitleFont = nil;
...
- (void)drawRect:(CGRect)rect {
    ...
    titleLabel.textAlignment = self.titleAlight;
    ...
    if (_subView) {
        ...
        CGFloat ratio = 0.9;
        CGFloat sizeWidth = _subView.frame.size.width > screenSize.width*ratio ? screenSize.width*ratio : _subView.frame.size.width;
        CGFloat buttonHeight = 0;
        titleLabel.frame = CGRectMake(8, marginTop, sizeWidth - 2*8, [titleLabel.text heightForWidth:sizeWidth-16 usingFont:titleLabel.font]);
        if (_numberOfButtons > 0) {
            buttonHeight = 45;
        }
  
        _subView.frame = CGRectMake(0,
                                    titleLabel.frame.size.height + marginTop,
                                    sizeWidth,
                                    _subviewSize.height);
        
        alertViewFrame = CGRectMake((screenSize.width - (sizeWidth))/2,
                                    (screenSize.height - _subView.frame.size.height)/2 - titleLabel.frame.size.height,
                                    sizeWidth,
                                    _subviewSize.height + titleLabel.frame.size.height + marginTop + buttonHeight);
...
- (void) showAlertWithTitle:(NSString *)title withSubView:(UIViewController *)subView andButtons:(NSArray *)buttons
{
    _subviewSize = subView.view.frame.size;
    [self setAlertViewAttributes:title withSubtitle:@"" withCustomImage:nil withDoneButtonTitle:nil andButtons:buttons];
