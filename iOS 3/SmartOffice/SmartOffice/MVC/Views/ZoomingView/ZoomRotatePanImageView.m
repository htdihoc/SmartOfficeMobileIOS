...
//    recogniser.scale = 1;
    ...
    if ([recogniser state] == UIGestureRecognizerStateBegan ||
        ...
        CGFloat currentScale = [[[recogniser view].layer valueForKeyPath:@"transform.scale"] floatValue];
        
        // Constants to adjust the max/min values of zoom
        const CGFloat kMaxScale = 2.0;
        const CGFloat kMinScale = 1.0;
        
        CGFloat newScale = 1 -  (lastScale - [recogniser scale]);
        newScale = MIN(newScale, kMaxScale / currentScale);
        newScale = MAX(newScale, kMinScale / currentScale);
        CGAffineTransform transform = CGAffineTransformScale([[recogniser view] transform], newScale, newScale);
        [recogniser view].transform = transform;
        
        lastScale = [recogniser scale];  // Store the previous scale factor for the next pinch gesture call
    }
}
