...
@interface KTTS_ConfirmProperty_iPad () <UITextViewDelegate, KPDropMenuSelectedDelegate>
...
- (void)viewDidLoad {
    ...
    self.view_kindOfReport.selectedDelegate = self;
//    self.view_kindOfReport.items = @[@"Báo không sử dụng", @"Báo mất", @"Báo hỏng"];
//    self.lbl_DefaultKindOfReport.text = @"Lựa chọn";
    ...
    [self.view_kindOfReport setTitleTextAlignment:NSTextAlignmentLeft];
...
#pragma mark KPDropMenuSelectedDelegate
- (void)itemSelected:(NSInteger)index {
    [self.lbl_DefaultKindOfReport setHidden:YES];
    switch (index) {
        case 0:
            _lbl_quantity.text          = @"Số lượng không dùng:";
            _lbl_unuseDays.text         = @"Ngày không dùng:";
            break;
        case 1:
            _lbl_quantity.text          = @"Số lượng mất:";
            _lbl_unuseDays.text         = @"Ngày mất";
            break;
        case 2:
            _lbl_quantity.text          = @"Số lượng hỏng:";
            _lbl_unuseDays.text         = @"Ngày hỏng:";
            break;
        default:
            break;
    }
